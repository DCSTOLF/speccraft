...
- **Build-repair mode is the ONE sanctioned carve-out in that rule, and it admits at most 10 edits per session (spec 0048).** When a Go production edit hits a build failure, the guard probes whether the *proposed* edit would leave the module compiling. If it would, the edit is allowed silently — a build failure must never block its own repair, which is what made spec 0047 spend five overrides against a stated budget of zero. If it would not, the edit is still allowed, but only after being recorded in an append-only session log, and the 11th such edit blocks, naming `/speccraft:spec:override`. The bound is session-wide: a clean probe ends repair mode but never refunds budget already spent, so break/fix cycling cannot evade it. Only Go has a prober; Python, JS/TS and Rust keep the unmodified blocking behaviour. The number here and the compiled cap are pinned to each other by a test — changing one without the other fails.
