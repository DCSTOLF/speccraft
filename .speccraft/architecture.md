...
- **Plugin-root resolution (spec 0030):** `speccraft-state plugin-root` is the single source of truth for a slash command's own install directory, resolving `SPECCRAFT_PLUGIN_ROOT` → validated `CLAUDE_PLUGIN_ROOT` → self-derivation (`os.Executable()` → `EvalSymlinks` → ascend to the manifest-identity-valid ancestor) in `tools/internal/speccraft/pluginroot.go`. Command `.md` bodies call it (bare, via `bin/`-on-`PATH`) instead of dereferencing `$CLAUDE_PLUGIN_ROOT`, which is only reliably exported to hooks — see conventions.md §"Command docs resolve the plugin root".
- **Stack-detection / authoring-surface boundary (spec 0034):** `DetectStack(root,
  cfg)` (`tools/internal/speccraft/detect.go`) is the single source of truth for
  the host repo's primary language, effective SUITE test command, and test-file
  globs, probing ONLY exact repo-root manifests with fixed precedence `go > rust >
  python > ts > js`. It READS the same config-backed commands the runners already
  use (`cfg.TDD.<Lang>.Command`; rust via `cfg.TDD.Rust.Runner`) — it does not
  change execution. Two `speccraft-state` subcommands surface it: `detect-stack`
  (versioned `{"schema":1,…}` JSON) and `test-command` (the effective command as
  data). The editable override is a single-line `<!-- speccraft:test-command =
  "…" -->` marker in the host repo's `.speccraft/conventions.md`, which WINS over
  detection; `/speccraft:init` seeds it (`commands/init.lib.sh::seed_conventions`,
  preserve-existing/idempotent). The marker parser lives in `package main`
  (`tools/cmd/speccraft-state/testcommand.go`) — reached only via the CLI, not a
  new internal symbol. Two meta-tests (`authoring_prose_test.go`,
  `template_purity_test.go`) make the "authoring layer + shipped template stay
  stack-neutral" guardrail executable. No new package; the detection code is a peer
  of the runner primitive (which owns EXECUTION), this one owns AUTHORING-layer
  reporting.
