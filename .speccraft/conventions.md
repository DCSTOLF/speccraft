...
- **New-symbol introduction still needs an override; a broken build no longer does (spec 0048).** A just-added test for a brand-new symbol cannot compile until the production edit lands, so the pre-edit run is a build failure (`OutcomeBuildFailed`), which is correctly *not* treated as RED. The sanctioned path for that first symbol-introduction edit remains a one-shot `/speccraft:spec:override`, same as Rust.
