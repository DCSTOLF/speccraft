...
When a feature is "model-driven but partly deterministic" — a heuristic that proposes something a human confirms (supersession detection, grouping, candidate matching) — split it so the **deterministic seed** lives in the bats/Go layer and only the genuinely fuzzy part (prose, semantic grouping) is left to the model + e2e tier. Don't punt the whole heuristic to "model behavior, structural-check-only."

- **Why.** A cross-model review (spec 0024, round 1) flagged that "supersession detection is a model step" gave the proposal *no deterministic test surface*. Extracting `history_supersession_seed` (explicit `supersedes:` markers + in-body `spec NNNN` cross-references, out-of-window only) moved the candidate-extraction logic to `tests/hooks/history-compact.bats` — pinned cheaply and exactly — while the model only does the thematic grouping/prose. The e2e then asserts the *seeded* outcome structurally (a `Supersedes:` line appears) rather than hoping the model invents one.
...
## Red-check capture for runtime-runner languages without a persisted baseline

Introduced by spec 0018.

Rust's red-check is backed by a persisted `rust_test_baseline` that attests a prior RED, so its "just-added" set is derived from a baseline and an empty just-added set safely *allows* a green/refactor edit. Go, Python, and JS/TS have **no such baseline**. For any future runtime-runner language in `speccraft-guard` that also lacks a persisted baseline, follow the capture-at-test-edit model rather than inventing a per-language baseline:

- **Capture at test-edit time, not at prod-edit time.** When the `IsTestFile` dispatch branch fires, compute the set of test identifiers this edit *introduced*: read the pre-edit on-disk content, model the post-edit content with the existing `applyEdit`, run a per-language identifier extractor over both, and take post − pre. Persist it keyed by the absolute test-file path into the single-writer `Session.RedCandidates map[string][]string` (`red_candidates,omitempty`). It is cleared on `SessionStart` like the rest of `Session`. Canonical: `captureRedCandidates` in `tools/cmd/speccraft-guard/main.go`, extractors in `tools/internal/speccraft/lang_testids.go`.
- **Empty just-added set must BLOCK, never allow.** This is the deliberate divergence from Rust's `if len(justAdded)==0 { return nil }`. Allowing-on-empty for a baseline-less language reopens the touch-check bypass (a blank-line edit introduces no identifier → empty set → must block). Do not copy Rust's allow-on-empty branch into a baseline-less language.
- **Fail closed on an unresolved runner.** Adapter resolution returns `(Runner, bool)`; `ok==false` must BLOCK with a "no test runner available — configure one or use `/speccraft:spec:override`" message, never fall back to a touch-check. Falling back re-opens the same bypass via an arranged-absent runner. Canonical: `runner.AdapterForLanguage`.
- **Bound the real invocation.** The runtime runner is invoked from an interactive PreToolUse hook; wrap it in `context.WithTimeout` (currently `const redCheckTimeout = 30 * time.Second`). A deadline overrun surfaces as a Go `error` from `adapter.Run` and BLOCKs — it is not a new `Outcome` enum value. The `Outcome` taxonomy (`OutcomeAllPassed`/`OutcomeAtLeastOneFailed`/`OutcomeBuildFailed`) does not grow for timeouts.
- **Known limitation: new-symbol introduction needs an override.** A just-added test for a brand-new symbol cannot compile until the production edit lands, so the pre-edit run is a build failure (`OutcomeBuildFailed`), which is correctly *not* treated as RED. The sanctioned path is a one-shot `/speccraft:spec:override` for that first symbol-introduction edit, same as Rust. Do not "fix" this by relaxing the build-failure-is-not-RED rule; the proper fix (apply-edit-in-memory red-check) is a deferred follow-up with its own ACs and latency profile.
...
Introduced by spec 0031.

When a failing test must justify a change to `speccraft-guard`'s OWN package — or ANY change where referencing the new symbol/signature would make the sibling test fail to COMPILE — author the RED at the JSON/decode boundary so it compiles against current code and fails on BEHAVIOR. This is the override-free path around the build-failure trap:

- **Why.** A `package main` test that references a not-yet-existing field or a new function signature does not COMPILE, so the runner reports `OutcomeBuildFailed`, which the guard correctly does NOT treat as a valid RED (see the new-symbol-bootstrap limitation above). The gated production edit is the one that would make it compile → the edit is blocked → an override is needed. That is the exact bootstrap trap spec 0031 exists to fix, so its own plan could not fall into it.
