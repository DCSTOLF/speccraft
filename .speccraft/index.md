...
- 2026-08-07 — Per-task done-means contract + mechanical completion gate (spec 0047, closed; unreleased at 1.16.0): `tasks.md` gains sub-checkboxes and a per-task `done:` line under `contract: done-means-v1`; `speccraft-state tasks-verify <tasks.md> [--run]` (a `run()`-seam case) exits 0/1/2 and is now `/speccraft:spec:close` step 2, ahead of the diff and memory-keeper. A `[x]` parent with a `[ ]` child fails on EVERY tasks.md; `done: $ <cmd>` predicates run under `/bin/sh -c` from the repo root with a process-group kill. Bypass needs the literal `SKIP-TASKS-VERIFY` — "approve all" does not. Override budget stated 0, **actual 5** (the guard wedges when a broken build makes it forbid its own repair). Two review rounds; round 1 produced a **false quorum** (both aux delegators wrote one temp path) — highest-priority follow-up.

- 2026-08-04 — Release 1.16.0: ledger write-locking closes the concurrent-writer race (spec 0045, closed; `related-specs: 0043, 0044`): every ledger writer now serializes its entire read-modify-write behind an exclusive advisory `flock` on `<workspace-root>/.speccraft/ledger.lock`, with the authoritative read moved inside the lock. `ledger-archive`'s two-file transaction is held under the same lock end to end. `--expect` stays a semantic CAS re-derived in-process from bytes read under the lock. Bounded by a `LOCK_NB` poll loop (never a cancellation goroutine) with `SPECCRAFT_LEDGER_LOCK_TIMEOUT` (default 10s); on contention stderr contains `ledger busy`. Crash-safe (advisory, kernel-released). Three-round cross-model; override budget 0.
