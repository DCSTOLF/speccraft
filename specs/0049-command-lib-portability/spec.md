...
10. `.github/workflows/ci.yml` runs the **full** `tests/hooks/` bats suite (all
    ...
    - **Every genuinely GNU-dependent test file is ported, not excluded.**
      **Amended during implementation (T13/T14).** This AC originally claimed
      "exactly one file, seven mutations". That was wrong, and the mechanised
      sweep required by carried review item 8 is what disproved it: the
      hand-written sweep behind the original claim searched for `grep -P ` with
      a trailing space and therefore missed `grep -qP`. The true set is **two
      files, nine call sites**:
      - `spec-revise-preflight.bats` — seven `sed -i 's/…/'` fixture
        mutations. `sed -i` with no suffix is GNU-only (BSD sed reads the next
        argument as the backup suffix). Ported to a local `sed_inplace` helper
        that filters to a temp and moves, avoiding in-place entirely.
      - `init-workspace.bats:139-140` — two `grep -qP` calls. `-P` is a
        GNU/PCRE extension absent from BSD grep. Ported to `printf`-produced
        tabs matched with `grep -qF`.
      Porting beats an exclusion list, which would need maintaining and would
      silently grow. The claim is now asserted mechanically by
      `tests/hooks/tests-portability-sweep.bats` rather than stated in prose,
      so the next such error fails a test instead of shipping.
      (`frontmatter-writer-guard.bats` and `portability-guard.bats` also match
      a naive grep, but their hits are fixture *strings* written to disk and
      never executed. The sweep's discriminator is "executed", not
      "mentioned", and its mention-only set is pinned as an exact list so
      reclassifying a real execution as a mention is a visible decision.)
