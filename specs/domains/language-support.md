...
- `DetectStack(root, cfg)` inspects only exact repo-root manifests and returns a typed `Stack{Language,TestCommand,TestPatterns,InlineTests}` mapping `go.mod`→go, `Cargo.toml`→rust (InlineTests, command from `cfg.TDD.Rust.Runner`), `pyproject.toml`/`setup.py`/`setup.cfg`/`requirements.txt`→python, `package.json`+`tsconfig.json`→ts else js, none→unknown; a polyglot root picks the primary by the fixed order go > rust > python > ts > js (spec 0034)
